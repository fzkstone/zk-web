...
module.exports = (router) => {
  ...
  .get((req, res, next) => {
    ...
    async.angelFall([
      ...
      (players, next) => {
        ...
        const result = _.first(_.sortBy(players, (player) => {
          if (isStrong) {
    ...
    async.angelFall([
      (next) => {
        async.parallel({
          ...
          player: (done) => {
            service.user.getPlayerById(targetPlayerId, done);
          }
        }, next);
      },
      (data, next) => {
        service.map.getMapById(data.player.playerMap, (err, mapData) => {
          if (err) {
            return next(err);
          }
          data.map = mapData;
      ...
      (data, next) => {
        const mapList = _.clone(data.map.mapNext || []);
        mapList.push(data.map.mapId);

        const playerStrongValue = service.user.calcStrong(data.player);

        const result = _.filter(data.players, (player) => {
          if (!_.contains(mapList, player.playerMap)) {
            return false;
          }
          if (isStrong) {
            return playerStrongValue > service.user.calcStrong(player);
          } else {
            return data.player[target] > player[target];
          }
        });
        return next(null, result);
      },
  ...
  .get((req, res, next) => {
    ...
    async.angelFall([
      ...
      (players, next) => {
        const result = _.take(_.sortBy(players, (player) => {
          if (isAscend) {
            return service.user.calcStrong(player);
          } else {
            return -service.user.calcStrong(player);
