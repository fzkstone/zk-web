  ...
  .get((req, res, next) => {
    ...
    async.angelFall([
      ...
      (data, next) => {
        ...
        _.each(data.player1.playerItems, (itemId) => {
          const val = itemMap[itemId] || 0;
          player1value = Math.max(player1value, val);
        });
        _.each(data.player2.playerItems, (itemId) => {
          const val = itemMap[itemId] || 0;
          player2value = Math.max(player1value, val);
        });
        ...
        async.parallel({
          player1: (done) => {
            service.user.updatePlayer(player1, { playerExp: player1exp }, done);
          },
          player2: (done) => {
            service.user.updatePlayer(player2, { playerExp: player2exp }, done);
          },
        }, next);
      },
      //userlog
